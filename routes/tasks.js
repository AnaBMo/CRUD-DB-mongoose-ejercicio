...
router.put("/tasks/:_id", async(req, res) => { 
    try {
        ...
        const task = await Task.findByIdAndUpdate(
            req.params._id, 
            { Taskname }, // objeto que queremos actualizar {Taskname : Taskname}, al llamarse igual no hay que ponerlos los dos. Pero la clave es lo que quiero poner y el valor es lo que nos da el cliente para actualizar.
            { new: true } // documento actualizado
        );
